"""Llama-specific story generator strategy using Markdown formatting.

Llama models (and similar instruct models like Dolphin) respond well to
clear Markdown structure with explicit instructions.
"""
...
class LlamaGeneratorStrategy(StoryGeneratorStrategy):
    """Strategy for Llama/Dolphin and similar Ollama models using Markdown."""
    ...
    def build_system_prompt(self, ctx: GeneratorContext) -> str:
        ...
        if size_tier in ("tiny", "small"):
            cot_instruction = (
                ...
                "YOU MUST think before writing JSON. Use this structure:\n\n"
                "```thinking\n"
                ...
                "```\n\n"
                "After your thinking block, output the JSON."
            )
            guidance_density = (
                "## Critical Rules\n\n"
        ...
        return (
            ...
            "## Description Style - Direct and Immersive\n\n"
            "Write DIRECT, IMMERSIVE descriptions. Avoid meta-language.\n\n"
            "BAD: 'A tavern described as having stale ale and unwashed bodies'\n"
            "BAD: 'A city known for its powerful merchant guilds'\n"
            "BAD: 'An artifact said to grant great power'\n\n"
            "GOOD: 'The tavern reeks of stale ale. Unwashed bodies press shoulder to shoulder.'\n"
            "GOOD: 'The merchant guilds rule the city from marble halls.'\n"
            "GOOD: 'The artifact grants power. It demands blood.'\n\n"
            "Rules:\n"
            "- NO 'described as having', 'known for', 'said to', 'legend speaks of'\n"
            "- Use concrete verbs: reeks, presses, stains, glistens, echoes\n"
            "- Lead with sensory details: smell, sound, texture, sight\n"
            "- 60-180 words per description, dense and atmospheric\n\n"
            f"{json_example}\n"
    ...
    def build_user_prompt(self, ctx: GeneratorContext) -> str:
        """Build Llama-formatted user prompt with Markdown structure."""
