...
def process_trigger(trigger: str, story: StoryModel | None = None) -> str:
...
def stream_trigger(trigger: str, story: StoryModel | None = None, db=None):
    start = time.monotonic()
    buffer = ""
    last_meta = {}
    try:
        ...
        for part in ollama_client.generate(model=OLLAMA_MODEL, prompt=prompt, stream=True):
...
def handle_turn(payload: TurnRequest, db=Depends(get_db)):
    ...
    result = process_trigger(payload.trigger, story)
    if story:
        update_story_summary(
            client=ollama_client,
            model=SUMMARY_MODEL,
...
def handle_turn_stream(payload: TurnRequest, db=Depends(get_db)):
    ...
    return StreamingResponse(stream_trigger(payload.trigger, story, db), media_type="text/plain")
